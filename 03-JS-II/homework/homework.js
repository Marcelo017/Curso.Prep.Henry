...
function mayoriaDeEdad(edad) {
  ...
  var allowed = (edad >= 18) ? 'Allowed' : 'Not allowed';
  return allowed;
}
...
function esEntero(numero) {
  ...
  // Tu código:
  if (Math.floor(numero) != Math.ceil(numero)) return false;
  return true;
}
...
function tablaDelSeis(){
  ...
  //Escribe tu código aquí   
  var aux = [];
  for (let i = 0; i <= 60; i++) if (i % 6 === 0) aux.push(i);
  return aux;
}
...
function tieneTresDigitos(numero){
  ...
  //Escribe tu código aquí
  var conversion = numero.toString()
  if (conversion.length === 3) return true;
  return false;
}
